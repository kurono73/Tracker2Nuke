...
def trackerNodeBuilder(name, data):
    ...
    ls = "} \n}\nname trackerFromBlender_@@@\n}\n"
    trfs0 = ' { {curve K x@@@ 1} '
    trls = '{curve K x@@@ 0} {curve K x@@@ 0} 1 1 1 {curve x@@@ 0} 1 0 -32 -32 32 32 -22 -22 22 22 {} {}  {}  {}  {}  {}  {}  {}  {}  {}  {}   } \n'

    trackerNode = fs.replace('@@@', str(len(data)))
    
    for k, v in data.items():
        if not v:
            continue
        sorted_frames = sorted(v.keys())
        if not sorted_frames:
            continue
        
        firstframe = sorted_frames[0]
        trackerNode += trfs0.replace('@@@', str(firstframe))
        trackerNode += f'"{k}" '
        
        trackerNode += '{curve ' + ' '.join(f'x{f} {v[f][0]}' for f in sorted_frames) + '} '
        trackerNode += '{curve ' + ' '.join(f'x{f} {v[f][1]}' for f in sorted_frames) + '} '
        trackerNode += trls.replace('@@@', str(firstframe))
        
    trackerNode += ls.replace('@@@', name.replace(' ', '_').replace('.', '_'))
    return trackerNode
...
class NUKE_OT_ExportPatternCorners(bpy.types.Operator):
    ...
    def execute(self, context):
        clip = context.space_data.clip
        tracking_object = get_active_tracking_object(context)
        selected = [t for t in tracking_object.tracks if t.select]
        if len(selected) != 1:
        ...
        for m in track.markers:
            frame = m.frame + clip.frame_start - 1
            center = (m.co[0] * clip.size[0], m.co[1] * clip.size[1])
            for i, offset in enumerate(m.pattern_corners):
                px = (offset[0] * clip.size[0], offset[1] * clip.size[1])
                corners[f"{track.name}_corner{i+1}"][frame] = (center[0] + px[0], center[1] + px[1])
...
class NUKE_OT_ExportPlaneTrack(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if not plane_track or not plane_track.markers:
            self.report({'INFO'}, "No valid plane track selected in the active object.")
            return {'CANCELLED'}
...
class NUKE_OT_CopyDistortion(bpy.types.Operator):
    ...
    def execute(self, context):
        ...
        if cam.distortion_model != 'NUKE':
            self.report({'WARNING'}, "Distortion model must be 'NUKE'.")
            return {'CANCELLED'}
...
class NUKE_OT_PasteDistortion(bpy.types.Operator):
    ...
    @classmethod
    def poll(cls, context):
        return bool(context.window_manager.clipboard and context.space_data and context.space_data.type == 'CLIP_EDITOR' and context.space_data.clip)

    def execute(self, context):
        clip = context.space_data.clip
        cam = clip.tracking.camera
        text = bpy.context.window_manager.clipboard
        k1 = re.search(r"distortionDenominator0\s+([-\d.]+)", text)
        k2 = re.search(r"distortionDenominator1\s+([-\d.]+)", text)
        if not (k1 and k2):
            self.report({'WARNING'}, "Clipboard does not contain valid data.")
            return {'CANCELLED'}
        cam.distortion_model = 'NUKE'
        cam.nuke_k1 = float(k1.group(1))
        cam.nuke_k2 = float(k2.group(1))
